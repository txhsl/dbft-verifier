...
pub fn verify_update_header(parent: Header, current: Header) -> bool {
    ...
    if current.extra_data.is_empty() {
        return false;
    }
    match current.extra_data[0] {
        EXTRA_V0 => {
            // Check format
            if current.extra_data.len()
            ...
            match encode_sig_header(current) {
                Ok(out) => {
                    // Verify sigs
                    verify_multi_sigs(keccak256(out), sigs, addrs)
                }
                Err(_) => false,
            }
        }
        EXTRA_V1 | EXTRA_V2 => {
            if current.extra_data.len() < 2 {
                return false;
            }
...
fn encode_sig_header(header: Header) -> Result<Vec<u8>, String> {
    ...
    let list_header = alloy_rlp::Header {
        list: true,
        payload_length: match header.extra_data[0] {
            EXTRA_V0 => {
                header_payload_length(header.clone()) - header.extra_data.length()
                    + hashable_extra_len
            }
            EXTRA_V1 => {
                header_payload_length(header.clone()) - header.extra_data.length()
...
fn verify_multi_sigs(hash: B256, sigs: Vec<Signature>, addrs: Vec<Address>) -> bool {
    ...
    for sig in sigs.iter().take(5) {
        signers.push(sig.recover_address_from_prehash(&hash).unwrap())
    }
    signers.sort();
    let mut vi = 0;
